...
press$Press.Liberty <- 100 - press$PRESS.LIBERTY.INDEX
...
sum_confirmed_country <- covid19_cases %>% 
  group_by(ISO) %>%  
  summarize(Confirmed = sum(Cases, na.rm=TRUE))
...
dataset_final$Death.Ratio.Population <- (dataset_final$Deaths / dataset_final$Population) * 1000000
dataset_final$Confirmed.Ratio.Population <- (dataset_final$Confirmed / dataset_final$Population) * 1000000
dataset_final$Death.Ratio.Confirmed <- (dataset_final$Deaths / dataset_final$Confirmed) * 100
```
